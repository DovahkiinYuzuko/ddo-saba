...
### `sendMessage` (L501-544)
- **役割:** ユーザーからの入力を受け取り、チャットセッションにメッセージを即時追加します（共有モードかどうかにかかわらず）。共有モードの場合はキューに参加し、推論の順番を待ちます。
- **引数:**
  - `inputText`: `string` - ユーザーが入力したテキスト。
- **戻り値:** `Promise<void>`

### `stopGeneration` (L578-582)
