...
### `sendMessage`
- **役割:** ユーザーからの入力を受け取ります。共有モード時は、メッセージを即座にチャット履歴に追加せず、状態マシンに `SUBMIT_MESSAGE` を送信して待機状態に入ります。非共有モード時は即座に推論をトリガーします。
- **引数:**
...
### `runInferenceStream`
- **役割:** 実際の推論ストリーム処理を実行し、UIのチャットログを更新しながらレスポンスを表示します。
- **引数:**
